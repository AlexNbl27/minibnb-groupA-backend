...
describe("Profile Routes", () => {
  ...
  describe("PATCH /api/v1/profiles/me", () => {
    it("should set avatar_url to null if empty string is provided", async () => {
      ...
      const response = await request(app).patch("/api/v1/profiles/me").send({ avatar_url: "" });
    ...
    it("should accept valid avatar url", async () => {
      ...
      const response = await request(app).patch("/api/v1/profiles/me").send({ avatar_url: "https://example.com/pic.jpg" });
    ...
    it("should set avatar_url to null if camelCase avatarUrl is empty string", async () => {
      ...
      const response = await request(app).patch("/api/v1/profiles/me").send({ avatarUrl: "" });
      ...
      // Ensure avatarUrl was removed from the payload
      const updateCall = mockUpdate.mock.calls[0][0];
      expect(updateCall).not.toHaveProperty("avatarUrl");
    });
  });
});
